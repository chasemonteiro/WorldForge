...
("QuikClot","Do not intentionally inflict Hemorrhage during the encounter. If your assigned weapon has innate bleed, do not add any extra bleed source.","True",1),
("Warm Heart","Do not intentionally inflict Frostbite during the encounter. If your assigned weapon has innate frost, do not add any extra frost source.","True",1),
("Antidote Inherent","Do not intentionally inflict Poison during the encounter.","True",1),
("Putrefaction Preventative","Do not intentionally inflict Scarlet Rot during the encounter.","True",1),
("Emissary of Insomnia","Do not intentionally inflict Sleep during the encounter.","True",1),
("Panacea","Do not intentionally add any status effect source beyond whatever your assigned weapon already has.","Grand",2),
...
("Apeshit Intermission","At some point after the boss reaches half health, both players must deal their next instance of damage with a throwable before resuming weapon attacks.","True",1),
...
("Three Strikes","If the team wipes three times on this boss, both assigned weapons are considered condemned and must be rerolled before attempt four.","Grand",2),
("The Wheel","If one player dies three times while using the same assigned weapon on this encounter, that player must Weapon Appeal it before the next serious attempt.","Grand",2),
...
"THE WHEEL: if a player reaches three deaths on this encounter with the same assigned weapon, that player's weapon must be rerolled before the next serious attempt.",
...
# Insert into TC_EXTRA_RITES before its closing bracket.
needle="]\n];\nconst TC_RITE_META"
if needle not in s:
    raise SystemExit('TC_EXTRA_RITES closing marker missing')
rite_js=''.join(f"\n ['{name.replace("'","\\'")}','{text.replace("'","\\'")}','{tier}',{favor}]," for name,text,tier,favor in extra_rites)
s=s.replace(needle, rite_js+"\n];\nconst TC_RITE_META",1)

# Insert Chaos strings before .map wrapper.
needle="]\n].map(text=>({text,favor:1}));"
if needle not in s:
    raise SystemExit('TC_EXTRA_CHAOS closing marker missing')
chaos_js=''.join("\n '"+x.replace("'","\\'")+"'," for x in extra_chaos)
s=s.replace(needle, chaos_js+"\n].map(text=>({text,favor:1}));",1)

# Keep the one-weapon assumption visible in the encounter rules UI if the marker exists.
