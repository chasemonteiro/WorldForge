...
(() => {
  ...
  function refreshPhysickAvailability(){
    const selects=[...document.querySelectorAll('[data-physick]')];
    for(const select of selects){
      const others=selects.filter(x=>x!==select&&x.value).map(x=>itemKey(x.value));
      for(const option of select.options){
        if(!option.value){option.disabled=false;continue;}
        const key=itemKey(option.value);
        option.disabled=!PHYSICK_DUPLICATES_ALLOWED.has(key)&&others.includes(key)&&option.value!==select.value;
      }
    }
  }
  ...
  function resultMarkup(build,raw){
    ...
    const ranked=variantsFor(build.weapon.weaponName).map(candidate=>{const a=calculate(candidate,build,false),b=calculate(candidate,build,true);return {raw:candidate,one:round(a.total),two:round(b.total)};}).sort((a,b)=>Math.max(b.one,b.two)-Math.max(a.one,a.two)),bestOne=ranked.reduce((best,row)=>!best||row.one>best.one?row:best,null),bestTwo=ranked.reduce((best,row)=>!best||row.two>best.two?row:best,null);
    ...
    ${ranked.length>1?`<div class="tc-affinity-head"><strong>Affinity comparison</strong><span>Tap any row to equip it</span></div><div class="tc-affinity-best"><div><span>Best 1H</span><b>${h(affinityLabel(bestOne.raw))}</b><strong>${bestOne.one}</strong></div><div><span>Best 2H</span><b>${h(affinityLabel(bestTwo.raw))}</b><strong>${bestTwo.two}</strong></div></div><div class="tc-affinity-table">${ranked.map((row,i)=>`<button type="button" class="tc-affinity-row ${row.raw.name===raw.name?'current':''}" data-affinity-pick="${h(row.raw.name)}"><span>${i+1}. ${h(affinityLabel(row.raw))}</span><small>1H</small><b>${row.one}</b><span></span><small>2H</small><b>${row.two}</b></button>`).join('')}</div><div class="tc-affinity-foot">Table order favors the stronger of each affinity’s displayed 1H/2H values; the two cards above show the actual best affinity for each wielding mode.</div>`:''}`;
  }
  ...
  function refreshResults(){applyClassConstraintsToInputs();const build=currentDraft();refreshBuildStatus(build);refreshTalismanAvailability();refreshPhysickAvailability();refreshWeaponSearchState();if(!tcWeaponData)return;populateWeaponControls(build);const raw=rawFor({...build,weapon:{...build.weapon,variantName:document.querySelector('#tcBuildAffinity')?.value||build.weapon.variantName}});build.weapon.variantName=raw?.name||'';const host=document.querySelector('#tcWeaponResults');if(host)host.innerHTML=resultMarkup(build,raw);}
  function renderBuild(){
    ...
    app.innerHTML=`<section class="tc-screen tc-build-screen">${screenTop('Tarnished Build')}
      ...
      <div class="tc-build-section"><div class="tc-build-section-head"><h2>Attributes</h2><span>Current in-game values</span></div>
        <div class="tc-build-origin tc-build-field"><label for="tcBuildClass">Starting class</label><select id="tcBuildClass"><option value="">Custom / keep current stats</option>${Object.entries(STARTING_CLASSES).map(([key,value])=>`<option value="${key}" ${build.startingClass===key?'selected':''}>${h(value.label)}</option>`).join('')}</select><small>Choosing a class fills its base stats and enforces that class’s minimum attributes. Level stays exactly as entered; if your stats imply a different level, Build will flag it and offer a one-tap sync. A mismatch can also be intentional if you entered temporary Great Rune or equipment stat bonuses.</small></div>
