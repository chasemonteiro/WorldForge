...
for(const name of ['tcWeaponRecallChoices','tcBuildMasterworkRecall'])vm.runInContext(html.match(new RegExp('function '+name+'\\([^]*?\\n\\}'))[0],ctx);
ctx.tcEncounterMutationLocked=s=>s.current.worldClears.length>0;
const recallState={smithing:{masterworkRecalls:[{id:'own',owner:'chase',weapon:'Sword',build:{name:'Sword'},used:false},{id:'other',owner:'morgan',weapon:'Spear',build:{name:'Spear'},used:false},{id:'used',owner:'chase',weapon:'Axe',build:{name:'Axe'},used:true}]},current:{id:'r',chase:{name:'Club'},morgan:{name:'Bow'},worldClears:[]}};
assert.equal(ctx.tcWeaponRecallChoices(recallState,'chase').length,1);
const swapped=ctx.tcBuildMasterworkRecall(recallState,'r','own','Club','chase','Chase');
assert.equal(swapped.current.chase.name,'Sword');assert.equal(swapped.current.morgan.name,'Bow');
assert.equal(swapped.smithing.masterworkRecalls[0].used,true);
assert.equal(ctx.tcBuildMasterworkRecall(swapped,'r','own','Sword','chase','Chase'),null);
assert.equal(ctx.tcBuildMasterworkRecall(recallState,'r','other','Club','chase','Chase'),null);
recallState.current.worldClears=['morgan'];
assert.equal(ctx.tcBuildMasterworkRecall(recallState,'r','own','Club','chase','Chase'),null);
console.log('PASS: weapon Recall choices, ownership, one-use spend and world-clear lock.');
