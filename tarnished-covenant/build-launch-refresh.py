...
# Preserve regional RNG after the exploration threshold. Physical prerequisites
# substitute only when the ordinary capstone roll actually succeeds, so boss
# three does not automatically start the critical-path chain.
...
# Bell Bearing contracts live in shared run state. Surface a newly commissioned
# contract once on each participating device, and surface its sanctioned
# status once as well, without turning presentation state into shared data.
runpy.run_path('tarnished-covenant/build-shared-smithing-contract-sync.py')

# Physick-bearing Minor Erdtree Avatars live outside the ordinary boss pool.
# A finite shared Erdtree Writ reward authorizes one reachable Avatar;
# legacy Avatar victories reduce the remaining issue cap for existing runs.
runpy.run_path('tarnished-covenant/build-erdtree-writs.py')

# Extra-boss Weapon Appeal penalties record the concrete boss killed.
runpy.run_path('tarnished-covenant/build-appeal-extra-boss-record.py')

# Weapon assignments respect acquisition bosses and Appeals remember rejects.
runpy.run_path('tarnished-covenant/build-weapon-accessibility.py')
...
runpy.run_path('tarnished-covenant/test-shared-smithing-contract-sync.py')
runpy.run_path('tarnished-covenant/test-erdtree-writs.py')
